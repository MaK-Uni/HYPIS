...
# ── WEATHER APIs ──────────────────────────────────────────────────────────────
# v8.2 FIXES:
#   ✔ CRITICAL — _safe_json(): validates response body BEFORE calling .json().
#     The old code did requests.get(...).json() in one chain; when the server
#     returns an empty body (HTTP 200 but 0 bytes — typical of a firewall/proxy
#     blocking the request), this raises JSONDecodeError which was silently
#     swallowed → None cached for 3600 s → "Weather unavailable" for 1 hour.
#   ✔ CRITICAL — get_current_weather() cache removed. Caching a None result
#     poisons every session for the TTL period with no retry path. The function
#     now runs on every page-load; successful results are stored in
#     st.session_state so the page doesn't re-fetch on widget interactions.
#   ✔ CRITICAL — All three functions now show full diagnostic output:
#     HTTP status code + raw response snippet (first 200 chars) so the user
#     can tell whether the issue is a firewall, a rate-limit (HTTP 429), or
#     a server error (HTTP 5xx).
#   ✔ get_forecast & get_historical_weather: same _safe_json() guard applied.

def _safe_fetch(url, timeout=15, label="API"):
    """
    Perform a GET request and return the parsed JSON dict.
    Raises a descriptive RuntimeError on any failure so callers can show
    a specific error message rather than a generic 'unavailable'.

    Failure modes handled explicitly:
      - Empty response body  → JSONDecodeError (most common: firewall block)
      - Non-2xx HTTP status  → requests.HTTPError
      - Timeout              → requests.Timeout
      - DNS / connection     → requests.ConnectionError
      - Bad JSON             → json.JSONDecodeError (re-raised with snippet)
    """
    try:
        resp = requests.get(url, timeout=timeout)
        # Show HTTP status immediately so firewall blocks are visible
        ...
        if not body:
            raise RuntimeError(
                f"{label} returned HTTP {resp.status_code} but with an EMPTY body. "
                "This usually means a firewall, proxy, or SSL-inspection device is "
                "intercepting the request before it reaches Open-Meteo. "
                "Check outbound internet access from your server, or try running "
                "the app locally / on Streamlit Community Cloud."
            )
        try:
            return resp.json()
        ...
        raise RuntimeError(
            f"{label} connection error — cannot reach {url[:50]}…  "
            f"Detail: {ce}"
        )


# NOTE: No @st.cache_data here — caching None poisons all sessions for 1 hour.
# Instead we use st.session_state as a lightweight per-session cache so widget
# interactions don't re-fire the network request unnecessarily.
def get_current_weather(lat, lon, elev):
    """
    Fetch today's weather from Open-Meteo forecast API.
    Returns a dict on success, or None on failure (with st.error already shown).
    Uses st.session_state keyed by location so the network call fires once per
    session (not once per widget interaction) without poisoning other sessions.
    """
    _sk = f"wx_cache_{lat}_{lon}"
    if _sk in st.session_state and st.session_state[_sk] is not None:
    ...
    url = (
        ...
        f"&forecast_days=1&timezone={TIMEZONE}"
    )
    try:
        r   = _safe_fetch(url, timeout=15, label="Open-Meteo /forecast (current)")
        cur = r.get("current", {})
        d   = r.get("daily", {})
        if not d:
            raise RuntimeError(
                "Open-Meteo /forecast returned JSON but 'daily' key is missing. "
        ...
        # Show the full diagnostic — no silent swallowing
        st.error(
            f"❌ **Current weather fetch failed for {lat}°, {lon}°**\n\n"
            f"{e}\n\n"
            "FAO-56 calculations will use default meteorological values. "
            "Results are still scientifically valid but not location-specific for today."
        )
        return None
    except Exception as e:
        st.error(
            f"❌ **Unexpected error fetching current weather:** "
            f"{type(e).__name__}: {e}"
        )
        return None
...
def get_forecast(_k, lat, lon, elev):
    ...
    try:
        r = _safe_fetch(url, timeout=15, label="Open-Meteo /forecast (7-day)")
        d = r.get("daily", {})
        if not d:
            st.warning(
                "⚠️ Open-Meteo 7-day forecast returned JSON but 'daily' key is missing. "
                f"Keys present: {list(r.keys())}"
            )
            return None
        rows = []
        for i in range(len(d["time"])):
            ...
            rows.append({
                ...
                "rh_mean":      round((rh_mx + rh_mn) / 2, 1),
                "wind":         round(wk, 3),
                "rs":           d["shortwave_radiation_sum"][i] or 18.,
        ...
        st.error(f"❌ **5-day forecast unavailable**\n\n{e}")
        return None
    except Exception as e:
        st.error(
            f"❌ **Unexpected error fetching 5-day forecast:** "
            f"{type(e).__name__}: {e}"
        )
        return None
...
def get_historical_weather(start_date, end_date, lat, lon):
    ...
    try:
        r = _safe_fetch(url, timeout=25, label="Open-Meteo ERA5 archive")
        d = r.get("daily", {})
        if not d:
            st.warning(
                "⚠️ ERA5 archive returned JSON but 'daily' key is missing. "
                f"Keys present: {list(r.keys())}"
            )
            return None
        ...
        st.error(f"❌ **ERA5 historical data unavailable**\n\n{e}")
        return None
    except Exception as e:
        st.error(
            f"❌ **Unexpected error fetching historical weather:** "
            f"{type(e).__name__}: {e}"
        )
        return None
...
# v8.2: get_current_weather() is no longer cached — session_state is used
# inside the function itself to avoid re-fetching on widget interactions
# while still allowing fresh data after a full page reload.
_ch = f"{datetime.now().strftime('%Y%m%d%H')}_{LAT}_{LON}"   # kept for get_forecast/get_historical cache keys

with st.spinner(f"📡 Fetching live weather for {SITE_NAME}…"):
    wx = get_current_weather(LAT, LON, ELEV)
